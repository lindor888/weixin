(function($) {
    $.fn.jquizzy = function(settings) {
        ...
        for (questionsIteratorIndex = 0; questionsIteratorIndex < config.questions.length; questionsIteratorIndex++) {
            contentFob += '<div class="slide-container"><div class="question-number">' + (questionsIteratorIndex + 1) + '/' + config.questions.length + '</div><div class="question" id="questiontitle'+questionsIteratorIndex+'">' + config.questions[questionsIteratorIndex].question + '</div><ul class="answers" id="question'+questionsIteratorIndex+'">';
            ...
            if (questionsIteratorIndex < config.questions.length - 1) {
                ...
                contentFob += '<div class="next final"><a class="nav-show-result" onclick="tijiao('+config.questions.length+')">完成</a></div>';
            }
            contentFob += '</div></div>';
        ...
        superContainer.find('.final').click(function() {
            ...
            resultSet = '<h2 class="qTitle">提交成功！</h2>' + shareButton + '<div class="jquizzy-clear"></div>' + resultSet + '<div class="jquizzy-clear"></div>';
...
function tijiao(num){
	...
	var options='';
	//debugger;
	var vals = $("ul li[class='selected']");
	for(var i=0;i<num;i++){
		if(i == num-1){
			options += vals[i].innerHTML;
		   }else{
			   options += vals[i].innerHTML+',';
		   }
		...
		$.ajax({   
	       url:'/weixin-studio/question/questionsubQuestion',
	        type: 'POST',   
	        async: false,
	        data:  {'receiveQuestionBean.receivetitleid':headtitleid,'receiveQuestionBean.receivequestionid':questionid,'receiveQuestionBean.receiveoptions':options,'receiveQuestionBean.optionids':optionids,'questionStat.openid':openId,'questionStat.nickname':nickname,'questionStat.headimg':headimg},
	       dataType: 'json',  
...
var headimg = '';
	// 获取答题基本信息
	$(function(){
		var url = 'http://localhost:8080/weixin-studio/question/getlistQuestion';
	...
	//获取用户信息	
		$.ajax({
			url:"http://localhost:8080/weixin-studio/interact/showUInteract?ids="+openId,
			type:"get",
			async:false,
			success:function(data){
				// 展示头像
		...
		$.ajax({
			...
			success:function(data){
				if (data.msg == "success")
				{    
					headtitleid = data.rows.headtitleid;
	...
	var diaochaSucessHandler = function(voteData){
		...
		for(var i=0;i<questions.length;i++){
		   ...
		   if(j == optionBean.length-1){
			   optionids += optionBean[j].optionid;
		    html +=    '"'+optionBean[j].opt+'"';
		   }else{
			   optionids += optionBean[j].optionid+',';
		     html +=    '"'+optionBean[j].opt+'",';
		   }
