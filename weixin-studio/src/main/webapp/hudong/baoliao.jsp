	...
	<div class="infobox">
	    ...
	    <div class="content">
			...
			<table class="form-table">
				<tbody>
					<tr>
						<th width="90px">热线标题</th>
					    <td><input type="text" value="" name="title" id="title" class="small-text" />&nbsp;<font color="red">*</font></td>
					    <th>热线内容</th>
 					...
 					<tr><td colspan="2"><input type="button" onclick="search()" value="搜索" class="regular-button" /></td></tr>
				</tbody>
			</table>
			</form>
		</div>
    ...
    <div class="infobox"> 
      ...
      <div class="content">
        <table class="list_table ">
           ...
           <div class="pages_bar" id="pages_bar"><a href="" >上一页</a><a class="current_page">1</a><a href="" >下一页</a></div>
...
<script type="text/javascript">	  
var parentid = location.href.split("?parentid=")[1];
var url="/weixin-studio/hudong/selectHudong";
if(parentid != undefined && parentid !=""){
   url="/weixin-studio/hudong/selectHudong?parentid="+parentid;
}
var currentPage = 0;
$(function(){
	getData();   
...
currentPage++;
 $.ajax({   
		       ...
		       success:function(data){ 
		    	   if(data.message == "success"){   
                     ...
                     for(var i=0;i<data.rows.length;i++){
                         ...
                         html += '<td>'+data.rows[i].nickname+'</td>';
                         html += '<td><img src="'+data.rows[i].headimgurl+'" style="width:35px;height:35px"></td>';
                         html += '<td>'+data.rows[i].city+'</td>';
                         html += '<td>'+data.rows[i].title+'</td>';
                         html += '<td>'+data.rows[i].content+'</td>';
                         if(data.rows[i].img_url == null || data.rows[i].img_url == '[]'){
                          html += '<td>无图片</td>';
                         }else{
                         html += '<td><img src="'+data.rows[i].img_url+'" style="width:35px;height:35px"></td>';
                         }
                         html += '<td>'+data.rows[i].ctime.replace("T"," ")+'</td>';
                     }
                         $("#content").html(html);
		    	   } 
			...
			var str ='';
			str += '<a href="" onclick="upPage()">上一页</a>';
			str += '<a class="current_page">'+currentPage+'</a>';
			str += '<a href="" onclick="getData()">下一页</a>';
			$("#pages_bar").html(str);
}
function upPage(){
currentPage--;
...
return;
          ...
          $.ajax({   
		       url:'/weixin-studio/hudong/selectHudong',
		        type: 'POST',   
		        async: false,
		        data:  {'currentPage':currentPage},
		       dataType: 'json',  
...
function search(){
   ...
   $.ajax({   
		       url:'/weixin-studio/hudong/searchHudong',
		        type: 'POST',   
		        async: false,
		        data:  {'baoliao.start':start,'baoliao.end':end,'baoliao.title':title,'baoliao.content':content},
		       dataType: 'json',  
		       success:function(data){ 
		    	   if(data.message == "success"){
