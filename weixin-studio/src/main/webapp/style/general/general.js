		...
		$.each(param_array,function(i,n){
			switch(n["name"]){
				...
				case "queryData.title":
					...
					break;
				case "queryData.startDate":
					$.extend(param,{"queryData.startDate":n["value"]});
					break;
				case "queryData.endDate":
					$.extend(param,{"queryData.endDate":n["value"]});
					break;
				case "queryData.souceType":
					$.extend(param,{"queryData.souceType":n["value"]});
					break;
				case "queryData.contentId":
					$.extend(param,{"queryData.contentId":n["value"]});
					break;
				case "queryData.actionType":
					$.extend(param,{"queryData.actionType":n["value"]});
					break;
			   case "queryData.groupName":
					$.extend(param,{"queryData.groupName":n["value"]});
					break;
			   case "queryData.type":
   ...
   function searchForm(formId,dataGridId,url){
       ...
       if(s2.length>0 && e2.length>0){
        ...
        if(s2_long > e2_long){
         ...
         }else if(formId=='followersInfFrom'){
