...
/*代理对象的鼠标样式和位置*/
function _6(e){
	var dragData = $.data(e.data.target,"draggable");
	var _7=dragData.options;
	var _8=dragData.options.proxyDOM;
	if(_8){
		_8.css("cursor",_7.cursor);
	}else{
		//alert(dragData.options.proxy)
		_8=$(e.data.target);
		dragData.handle.css("cursor",_7.cursor);
	}
	_8.css({left:e.data.left,top:e.data.top});
};
/*返回可放置对象 add by wangwei >16*/
function setDroppable(target){
	var _b=$($.fn.droppable.defaults.module).filter(function(){
			return target!=this;		/*不能是本身*/
		}).filter(function(){
			try{
				var _c=$.data(this,"droppable").options.accept;
			}catch(e){
				//alert(e.description);
				//alert($.data(this,"droppable"))
				//alert($(this).html());
				//alert($(this).attr("id"));
				//return;
			}
			...
			if(_c){
				return $(_c).filter(function(){
					return this==target;/*accept指定的放置对象*/
				}).length>0;
			}else{
				return true;
		}
	});
	$.data(target,"draggable").droppables=_b;
	return target;
}
/*获取放置对象，构造代理对象，触发拖拽开始事件*/
function _9(e){
	var _a=$.data(e.data.target,"draggable").options;
	setDroppable(e.data.target);
	
	var _d=$.data(e.data.target,"draggable").options.proxyDOM;
	if(!_d){
		if(_a.proxy){
			if(_a.proxy=="clone"){
				/*modify by wangwei >1*/
				_d=$(e.data.target).clone().css({width:$(e.data.target).width()}).insertAfter(e.data.target);
			}else{
				_d=_a.proxy.call(e.data.target,e.data.target);
			}
			$.data(e.data.target,"draggable").options.proxyDOM=_d;
		}else{
			_d=$(e.data.target);
		}
	}
...
function _e(e){
	var dragData = $.data(e.data.target,"draggable");
	//console.log(!arguments.callee.last ||new Date().getTime() - arguments.callee.last)
	if (!arguments.callee.last ||new Date().getTime() - arguments.callee.last>30){
		//console.log("begin:" + gg++ + " target:" + $(e.data.target).attr("id"));
		_1(e);	//计算位置
		if(dragData.options.onDrag.call(e.data.target,e)!=false){
			_6(e);//移动
		}
		var _f=e.data.target;
		/*modify by wangwei > 3*/
		if (!dragData.droppables){
			setDroppable(e.data.target);
		}
		/*识别鼠标所指的draggable对象，并触发dragenter 或者 dragleave*/
		dragData.droppables.each(function(){
			var _10=$(this);
			var p2=$(this).offset();
		...
		arguments.callee.last = new Date().getTime();
		return false;
	} else {
		_1(e);	
		_6(e);
...
function _11(e){
	_1(e);
	var _12=$.data(e.data.target,"draggable").options.proxyDOM;
	...
	function _15(){
		...
		$.data(e.data.target,"draggable").options.proxyDOM=null;
	};
	
	/*识别鼠标所指的draggable对象，并触发drop事件*/
	function _14(){
		var _16=false;
		/*modify by wangwei >3*/
		if (!$.data(e.data.target,"draggable").droppables){
			setDroppable(e.data.target);
		}
...
$.fn.draggable.methods={options:function(jq){
	...
	return $.data(jq[0],"draggable").options.proxyDOM;
},enable:function(jq){
...
$.fn.draggable.defaults={proxyDOM:null,proxy:null,revert:false,cursor:"move",deltaX:null,deltaY:null,handle:null,disabled:false,edge:0,axis:null,
...
return false;
      ...
      $("span.icon-pindao", _8a).unbind(".tree").bind("click.tree", function() {
        	_da(_88,$(this).parent()[0],function(){},{treeType:4});
            return false;
        });
         $("span.icon-zhuanti", _8a).unbind(".tree").bind("click.tree", function() {
         	_da(_88,$(this).parent()[0],function(){},{treeType:2});
            return false;
        });
         $("span.icon-lanmu", _8a).unbind(".tree").bind("click.tree", function() {
         	_da(_88,$(this).parent()[0],function(){},{treeType:5});
            return false;
        });
         $("span.icon-kuaifan", _8a).unbind(".tree").bind("click.tree", function() {
         	_da(_88,$(this).parent()[0],function(){},{treeType:6});
            return false;
        });
         $("span.icon-cmssearch", _8a).unbind(".tree").bind("click.tree", function() {
         	var tree = _88;
         	var target = $(this).parent()[0];
         	$.messager.prompt("查询","输入名称",function(name){
         					if(name){
         						_da(tree,target,function(){},{name:name,treeType:"all"});
         						$(tree).parent().parent().show();
         					}
         				}
         		)
            return false;
        });
        $("span.icon-nodeAll", _8a).unbind(".tree").bind("click.tree", function() {
         	_da(_88,$(this).parent()[0],function(){},{nodeAll:true});
            return false;
        });
};
function _8d(_8e){
...
$("<span class=\"tree-title\"></span>").html(_ce.text).appendTo(_cf);
     if(_ce.id.substring(0,1)=="C"){
          if(_c6.search){
         	 $("<span class=\"icon-cmssearch\"></span>").prependTo(_cf);
          }
          if(_c6.kuaifan){
          	$("<span class=\"icon-kuaifan\"></span>").html("快").prependTo(_cf);
          }
          if(_c6.pindao){
          	$("<span class=\"icon-pindao\"></span>").html("<font color=red>频</font>").prependTo(_cf);
          }
          if(_c6.lanmu){
          	$("<span class=\"icon-lanmu\"></span>").html("<font color=green>栏</font>").prependTo(_cf);
          }
          if(_c6.zhuanti){
          	 $("<span class=\"icon-zhuanti\"></span>").html("<font color=blue>专</font>").prependTo(_cf);
          }
       }else if(_ce.id.substring(0,1)=="T" ){
       		if(_c6.nodeAll){
         	  $("<span class=\"icon-nodeAll\"></span>").prependTo(_cf);
            }
       }
...
if(_d3.nodeAll==true){
	$.extend(_d3,{nodeType:"pagewithchildren"})
	delete _d3.nodeAll;
}
...
function _da(_db,_dc,_dd,param){
...
if(hit.hasClass("tree-expanded")&&$.isEmptyObject(param)){
return;
}
var _df=_b1(_db,_dc);
...
var ul=$(_dc).next();
if(ul.length&&$.isEmptyObject(param)){
if(_de.animate){
...
}else if(!ul.length ||(ul.length&&!$.isEmptyObject(param))){
var _e0=ul.length?$(_dc).next():$("<ul style=\"display:none\"></ul>").insertAfter(_dc);
_d1(_db,_e0[0],$.extend({id:_df.id},param),function(){
if(_de.animate){
