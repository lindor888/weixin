...
_6c=_6c.replace(/{from}/,_6a.total == 0 ? 0 :_6a.pageSize*(_6a.pageNumber-1)+1); 
		...
		$("span.icon-pindao", _8a).unbind(".tree").bind("click.tree", function() {
         ...
         $("span.icon-cmssearch", _8a).unbind(".tree").bind("click.tree", function() {
         	...
         	$.messager.prompt("查询","输入名称",function(name){
         					if(name){
         						_be(tree,target,{name:name,treeType:"all"});
         ...
         $("span.icon-nodeAll", _8a).unbind(".tree").bind("click.tree", function() {
         	_be(_88,$(this).parent()[0],{nodeAll:true});
            return false;
        });
};
...
$("<span class=\"tree-title\"></span>").html(_b2.text).appendTo(_b3);
    if(_b2.id.substring(0,1)=="C"){
         if(_aa.search){
         	 $("<span class=\"icon-cmssearch\"></span>").prependTo(_b3);
          }
          if(_aa.kuaifan){
          	$("<span class=\"icon-kuaifan\"></span>").html("快").prependTo(_b3);
          }
          if(_aa.pindao){
          	$("<span class=\"icon-pindao\"></span>").html("<font color=red>频</font>").prependTo(_b3);
          }
          if(_aa.lanmu){
          	$("<span class=\"icon-lanmu\"></span>").html("<font color=green>栏</font>").prependTo(_b3);
          }
          if(_aa.zhuanti){
          	 $("<span class=\"icon-zhuanti\"></span>").html("<font color=blue>专</font>").prependTo(_b3);
          }
       }else if(_b2.id.substring(0,1)=="T" ){
       		if(_aa.nodeAll){
         	  $("<span class=\"icon-nodeAll\"></span>").prependTo(_b3);
            }
       }
if(_aa.checkbox){
...
if(_b7.nodeAll==true){
	$.extend(_b7,{nodeType:"pagewithchildren"})
	delete _b7.nodeAll;
}
...
function _be(_bf, _c0,param) {
        ...
        if (ul.length&&$.isEmptyObject(param)) {
            ...
            var _c4 = ul.length?_c2.next():$("<ul style=\"display:none\"></ul>").insertAfter(_c2);
            _b5(_bf, _c4[0], $.extend({id: _c3.id},param), function() {
                if (_c1.animate) {
...
win.window({title:_1a8,width:400,height:"auto",modal:true,collapsible:false,minimizable:false,maximizable:false,resizable:false,onClose:function(){
setTimeout(function(){
win.window("destroy");
},100);
}});
...
$.ajax({type:opts.method,url:opts.url,data:_4f9,dataType:"json",success:function(data){
	var tdata = [];
	if(!_4f8 && opts.pagination==true){//根节点数据
		$.data(_4f7,"treegrid")._data = data;
		opts.pageNumber = opts.pageNumber?opts.pageNumber:1;
		$(_4f7).datagrid("getPager").pagination({
			total:data.length,
			...
			onSelectPage:function(pageNumber,pageSize){
				var data = [];
				var _data = $.data(_4f7,"treegrid")._data;
				var opts = $.data(_4f7,"treegrid").options;
				opts.pageNumber = pageNumber;
				opts.pangeSize = pageSize;
				data = _data.slice((pageNumber-1)*pageSize, pageNumber*opts.pageSize)
				_4db(_4f7,null,data);
				opts.onSelectPage?opts.onSelectPage.call(_4f7,pageNumber,pageSize):false
			}
		})
		$(_4f7).datagrid("getPager").find("input.pagination-num")
		.unbind(".pagination").bind("keydown.pagination",function(e){
				if(e.keyCode==13){
					var pages=Math.ceil(data.length/opts.pageSize);
					var _60=parseInt($(this).val())||1;
					_60 = _60<=pages?_60:pages;
					opts.pageNumber=_60;
				}
			})
		tdata = data.slice((opts.pageNumber-1)*opts.pageSize,opts.pageNumber*opts.pageSize)
	}else {
		tdata = data
	}
	_4fc.removeClass("tree-loading");
	_4db(_4f7,_4f8,tdata,_4fa);
	if(_4fb){
	_4fb();
}
},error:function(){
...
return jq.each(function(){
	var data = [];
	var _data = $.data(this,"treegrid")._data;
	var opts = $.data(this,"treegrid").options;
	data = _data.slice(opts.pageNumber*opts.pageSize, opts.pageNumber*opts.pageSize+opts.pageSize)
	if(data.length>0){
...
return jq.each(function(){
	...
	data = _data.slice((opts.pageNumber-1)*opts.pageSize, (opts.pageNumber-1)*opts.pageSize+opts.pageSize)
	_4db(this,null,data);
});
},getData:function(jq){
...
_598=_598||{};
$.ajax({url:opts.url,type:opts.method,dataType:"json",data:_598,success:function(data){
_591(_597,data,_599);
},error:function(){
...
$.fn.combobox.defaults=$.extend({},$.fn.combo.defaults,{method:"get",valueField:"value",textField:"text",mode:"local",url:null,data:null,keyHandler:{up:function(){
_578(this);
},down:function(){
_57e(this);
},enter:function(){
