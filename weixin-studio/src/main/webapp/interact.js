...
//			url:'/weixin-studio/account/selectAllAccount',
	        ...
	        columns:[
	            [
		            ...
		            {field:'flagx',title:'选择状态',width:80,align:'center',sortable:true,
		            	formatter:function(value,rowData,rowIndex){
		            		switch(value){
		            			case 0: return "未选择"; break;
		            			case 1: return "<a style='color:red'>已选择</a>"; break;
		            ...
		            {field:'id',title:'操作',width:160,sortable:true,align:'center',sortable:true,
		            	formatter:function(value,rowData,rowIndex){
							...
							html += '<a href="javascript:xuanze(\''+rowData.id+'\')">选择</a>&nbsp;&nbsp;';
			...
			},
			{
				text:'查看已选内容',
					...
					disabled:false,
					handler:function(){
						window.location="/weixin-studio/program/bianjiInteract";
					}
			},
			{
				text:'查看节目单',
					...
					disabled:false,
					handler:function(){
						window.location="/weixin-studio/program/jiemudanViewInteract";
					}
			}
...
function xuanze(id){
	$.ajax({   
       url:'/weixin-studio/program/sessionAddInteract',
        type: 'POST',   
        async: false,
        data:  {'bean.flag':1,'bean.id':id},
       dataType: 'json',  
       success:function(data){ 
    	   if(data.msg=="success"){
    		   ...
    		   $.messager.alert('提示','选择失败,您可能已经选择过,请在已选内容中查看！');
    	   }
        }   
	});
}
