	...
	//验证初始化
		...
		$(document).ready(function(){
			...
			$("#userInfFrom").validate({   
				rules:{
				"waccount.name":{required:true,remote:{url:"/weixin-studio/account/checkwaccounttabwaccount?waccount.waccountId="+$("#waccountId").val(),
     ...
     var v=$("#welcomeContent").val();
    
     if(v.indexOf('Grap')!=-1){
    	 
    	
    	 $("#grap1").attr("selected","selected");

     }else{
    	 $("#text1").attr("selected","selected");
    	 
     }
     var no=$("#noReplyContent").val();
     if(no.indexOf('Grap')!=-1){
    	 $("#grap2").attr("selected","selected");
     }else{
    	 $("#text2").attr("selected","selected");
     }
     changeType();
	});
...
<div class="h" >
   ...
   <s:textfield type="hidden" name="waccount.type"/>
	<s:form  method="post" namespace="/account" action="updatewaccounttabwaccount"  id="userInfFrom"   theme="simple">
	<s:textfield id="waccountId" type="hidden" name="waccount.waccountId"/>  
	<p><i>*</i>&nbsp;<span style="padding-right: 12px;">微信公众平台用户名:</span><s:textfield id="waccountname" name="waccount.name" size="40"/></p>
	...
	<p><span style="padding-right: 60px;">公共号类型：</span> 
 	...
 	<s:radio    id="radtype"  list="#{'0':'订阅号&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;', '1':'服务号  '}" required="true" requiredposition="right"  name="waccount.type"  onclick="changeType()"></s:radio></p>
	...
	<select id="welcomeContented" onchange="qiehuan('comtitle');" >
	<option id="text1" value='1'>文本</option>
	<option id="grap1" value='2'>图文新闻</option>
	</select>
	...
	<span style="font-size: 10px;"><a href="javascript:void(0);" onclick="choosematerial();">挑选素材</a></span></p>
	<p><span style="padding-right: 15px;">无返回结果提示语：</span>
	<select id="noreplyed" onchange="qiehuan('notitle');"  >
	<option id="text2" value='1'>文本</option>
	<option id="grap2" value='2'>图文新闻</option>
	</select>
	<s:textfield id="notitle" readonly = "true" size="25" name="waccount.nocatalogTitle"/>&nbsp;
	<s:textfield id="noReplyContent" name="waccount.noReplyContent" type="hidden" />
	<span style="font-size: 10px;"><a href="javascript:void(0);" onclick="nochoosematerial();">挑选素材</a></span></p>
	<%-- <p ><span style="padding-right: 45px;">是否压缩图片：</span> 
	<s:radio list="#{'0':'是&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;', '1':'否'}" required="true" requiredposition="right"  name="waccount.ifCompressImage" ></s:radio></p> --%>
	<p><span style="padding-right: 125px;">url:</span><s:textfield readonly = "true" id="url" name="waccount.url" size="40"/> &nbsp; <input type="button"  onclick="createurl()" value="生成" style="width: 60px; height: 25px;"/></span></p>
	<div id="all"><div id="appidandsecret">
	<p id="appid"><i>*</i>&nbsp;<span style="padding-right: 90px;">AppId:</span><s:textfield id="waccountappid" name="waccount.appId" size="40"/>&nbsp;<span id="appidvalue" style="font-size: 10px;">公众平台申请到的AppId</span></p>
	<p id="appsecret"><i>*</i>&nbsp;<span style="padding-right: 53px;">Appsecret：</span><s:textfield id="waccountsecret" name="waccount.appSecret" size="40"/>&nbsp;<span id="appsecretvalue" style="font-size: 10px;">公众平台申请到的AppSecret<span></p>
	</div>
	<div>
	...
	<s:textfield id="hiddentappid" name="waccount.appId" type="hidden"/>
	<s:textfield id="hiddenappSecret" name="waccount.appSecret" type="hidden"/>
	...
	<div id="choosewaccountText" class="easyui-dialog" style="width:520px;height:450px;padding:10px 20px"
     buttons="#dlg-buttons"  >
			...
			<input type="button" value="查询" onclick="queryText();javascript: $('#contentInfo').datagrid('unselectAll');">
			<table id="textInfo">
			</table>
			<br/>
